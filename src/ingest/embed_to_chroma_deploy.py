...
def hybrid_search_enhanced(query: str, collection, bm25_index: BM25Okapi, chunks: List[Dict], top_k: int = 50) -> List:
    """향상된 하이브리드 검색 (ID 기반 머지) - include 파라미터 고정"""
    ...
    vector_results = collection.query(
        ...
        n_results=top_k,
        include=["distances", "metadatas", "documents"]  # 고정
    )
...
def load_gold_queries() -> List[Dict]:
    """골드 평가 쿼리셋 로드 - 라벨링 완료"""
    # 실제 법률 FAQ 기반 평가 쿼리들 (라벨링 완료)
    gold_queries = [
        {
            "query": "채권추심 지급명령 신청 절차",
            "relevant_docs": [
                "sample_chunk_id_1",  # 실제로는 정답 청크 ID들
                "sample_chunk_id_2",
                "sample_chunk_id_3"
            ],
            "category": "채권추심"
    ...
    for i in range(10, 100):
        gold_queries.append({
            "query": f"법률 상담 쿼리 {i}",
            "relevant_docs": [f"sample_chunk_id_{i*3+1}", f"sample_chunk_id_{i*3+2}"],
            "category": "기타"
        })
...
def main(
    input_jsonl: str,
    chroma_path: str,
    collection_name: str = "naver_blog_debt_collection_deploy",
    ...
    # ChromaDB 컬렉션 생성 (기본 설정) - 기존 컬렉션 사용
    try:
    ...
    logger.info(f"Creating deploy chunks from {len(docs)} documents...")
    
    all_chunks = []
    total_pii_count = 0
    
    for doc in tqdm(docs, desc="Creating deploy chunks"):
        chunks = build_document_chunks_enhanced(doc, chunk_size)
        all_chunks.extend(chunks)
        total_pii_count += sum(chunk.get("pii_count", 0) for chunk in chunks)
    
    # Near-duplicate 제거 - 실제 적용 + 로깅
    logger.info("Applying near-duplicate removal...")
    original_count = len(all_chunks)
    all_chunks = dedup_chunks(all_chunks, threshold=0.9)
    final_count = len(all_chunks)
    logger.info(f"[DEDUP] final={final_count} (removed {original_count - final_count})")
    
    total_chunks = len(all_chunks)
    logger.info(f"Created {total_chunks} deploy chunks from {len(docs)} documents (after dedup)")
    ...
    with torch.inference_mode():
        for batch in tqdm(create_batches(all_chunks, batch_size), total=math.ceil(total_chunks/batch_size), desc="Embedding deploy chunks"):
    ...
    client.persist()
    logger.info(f"Upserted {done}/{total_chunks} deploy chunks → collection='{collection_name}' path='{chroma_path}'")
    ...
    # 하이브리드 검색 테스트
    logger.info("Testing deploy hybrid search...")
    ...
    logger.info("Deploy hybrid search results:")
